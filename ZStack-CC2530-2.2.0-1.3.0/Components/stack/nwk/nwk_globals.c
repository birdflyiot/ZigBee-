...
{ uint16 temp1;
  ...
  memcpy(&IEEBUF[0],"IEEE: ",6);

  ch = (temp1 >> 12) & 0x0F;
  IEEBUF[j++] = ch + (( ch < 10 ) ? '0' : '7');
  ch = (temp1 >> 8) & 0x0F;
  IEEBUF[j++] = ch + (( ch < 10 ) ? '0' : '7'); 
  ch = (temp1 >> 4) & 0x0F;
  IEEBUF[j++] = ch + (( ch < 10 ) ? '0' : '7'); 
  ...
  switch ( statusCode )
  {
    case NWK_STATUS_COORD_ADDR:
      if ( ZSTACK_ROUTER_BUILD )
      {
        ...
        HalLcdWriteString(&IEEBUF[0],  HAL_LCD_LINE_2);
    ...
    case NWK_STATUS_ROUTER_ADDR:
      if ( ZSTACK_ROUTER_BUILD )
      {
        HalLcdWriteString( (char*)RouterStr, HAL_LCD_LINE_1 );
        HalLcdWriteString( &IEEBUF[0],  HAL_LCD_LINE_2);
      }
      break;
    ...
    case NWK_STATUS_ED_ADDR:
      ...
      { 
        HalLcdWriteString( (char*)EndDeviceStr, HAL_LCD_LINE_1 );
        HalLcdWriteString( &IEEBUF[0],  HAL_LCD_LINE_2);
      }
      break;
